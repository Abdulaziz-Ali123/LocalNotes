...
const FileSystemTree = forwardRef<FileSystemTreeRef, FileSystemTreeProps>(
  ({ onFileSelect, autoOpen = true, isVisible = true }, ref) => {
    ...
    const handleDrop = async (
      ...
      e.stopPropagation();
      // Support two cases:
      // 1) Internal move within the app (dragged item provides a text/plain source path)
      // 2) External OS file/folder drop (e.dataTransfer.files contains filesystem entries)

      // First, handle external file/folder drops from the OS
      ...
      if (files.length > 0) {
        ...
        try {
          for (const f of files) {
            // In Electron, file objects from OS include a `path` property
            const srcPath = (f as any).path as string;
            if (!srcPath) continue;

            const isDirRes = await window.fs.isDirectory(srcPath);
            if (isDirRes && isDirRes.data) {
              // Import whole folder into current folder
              const importRes = await window.fs.importFolder(srcPath, currentFolder);
              if (!importRes.success) {
                console.error("Import folder failed:", importRes.error);
                alert(`Failed to import folder: ${importRes.error}`);
              }
            } else {
              // Copy file into current folder, avoid collisions by finding a unique name
              const baseName = window.fs.basename(srcPath);
              const ext = window.fs.extname ? window.fs.extname(baseName) : baseName.substring(baseName.lastIndexOf('.'));
              const nameOnly = ext ? baseName.slice(0, baseName.length - ext.length) : baseName;
              let finalName = baseName;
              let candidate = window.fs.join(currentFolder, finalName);
              let existsRes = await window.fs.exists(candidate);
              let i = 1;
              while (existsRes && existsRes.success && existsRes.data) {
                finalName = `${nameOnly} (${i})${ext}`;
                candidate = window.fs.join(currentFolder, finalName);
                existsRes = await window.fs.exists(candidate);
                i++;
                if (i > 1000) break;
              }

              const copyRes = await window.fs.copyFile(srcPath, candidate);
              if (!copyRes.success) {
                console.error("Copy failed:", copyRes.error);
                alert(`Failed to copy file: ${copyRes.error}`);
              }
            }
          }

          // Refresh the root folder
          const root = rootPath || localStorage.getItem("currentFolderPath") || "";
          setLoadedFolders((prev) => {
            const set = new Set(prev);
            set.delete(root);
            return set;
          });
          if (root) await loadDirectory(root);
        } catch (err) {
          console.error("Error importing dropped files:", err);
          alert("Failed to import dropped files.");
        }

        return;
      }

      // Otherwise handle internal moves
