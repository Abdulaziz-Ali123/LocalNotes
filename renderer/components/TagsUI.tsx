...
type TreeContextProps = {
  ...
  direction: "rtl" | "ltr";
  onContextMenu?: (itemPath: string) => void;
  setContextMenu?: React.Dispatch<React.SetStateAction<{ x: number; y: number; itemPath: string } | null>>;
};
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    ...
    const [contextMenu, setContextMenu] = useState<{ x: number; y: number; itemPath: string } | null>(null);
    ...
    return (
      ...
      >
        <div className={cn("size-full truncate", className)}>
          <ScrollArea ref={ref} className="relative h-full px-2" dir={dir as Direction} onClick={() => setContextMenu(null)}>
        ...
        {contextMenu && (
          ...
          >
            <button
              ...
              onClick={() => {
                onContextMenu?.(contextMenu.itemPath);
                setContextMenu(null);
              }}
            >
              Add Tags
...
type FolderComponentProps = Omit<FolderProps & React.HTMLAttributes<HTMLDivElement>, 'tagIndicators' | 'itemPath'> & {
  tagIndicators?: React.ReactNode;
  itemPath?: string;
};

const Folder = forwardRef<HTMLDivElement, FolderComponentProps>(
...
>(
  (
    { value, className, handleSelect, isSelectable = true, isSelect, fileIcon, itemPath, children, ...props },
    ref
  ) => {
    ...
    return (
      <button
        ...
        {...props}
      >
        {fileIcon ?? <FileIcon className="size-7" />}
        {children}
      </button>
    );
  }
